...
```{r fig1, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.cap="Evolución del proceso dado por $Z_t =2+0.9Z_{t-1}$", fig.align='center', fig.height=3.5}
...
```{r fig2, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.cap="Evolución del proceso dado por $Z_t =2-0.5Z_{t-1}$", fig.align='center', fig.height=3.5}
...
```{r fig3, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.cap="Evolución del proceso dado por $Z_t =3+0.9Z_{t-1}-0.2Z_{t-2}$", fig.align='center', fig.height=3.5}
...
Para $\forall i \neq j$ y $i, j = 1, 2, \ldots, p$. Cuando $g_1 = g_2 = \ldots = g_p = g$, la condición de la ecuación \@ref(eq:COND3) se resume a que $|g| < 1$. En resumen, las condiciones descritas en las ecuaciones \@ref(eq:COND3) y \@ref(eq:COND4) se puden ilustrar con un circulo unitario como el de la Figura \@ref(fig:fig4) en que sí las raíces se ubican dentro de éste, podemos decir que el proceso es convergente en el largo plazo.

```{r fig4,fig.cap="Circulo unitario en el que se cumple que $|g_i|<1$ y $(g_i g_j)^{1/2} = (u^2 + v^2)^{1/2} < 1$", echo=FALSE, fig.height=4.5, fig.width=4, message=FALSE, warning=FALSE, fig.align='center'}
x = seq(-1,1,0.001)
y = sqrt(1-x^2)
par(new = T) 
...
\begin{figure}
  ...
  \caption{Circulo unitario en el que se cumple que: $\abs{g_i} < 1$ y $\sqrt{g_i g_j} = \sqrt{u^2 + v^2} < 1$}
  \label{Circle}
\end{figure}
...
Donde $\alpha_i$ puede ser remplazada por cualquier constante $a_i$, con $i = 1, 2, 3, \ldots$, para escribir ecuaciones como las anteriores. Adicionalmente, podemos decir que la ecuación (\ref{Ec_p}) es una generalización del caso de medias móviles, el cual admite una poderación distinta para cada uno de los elementos rezagados. 
...
Tales polinomios del operador rezago también son lladamos \textit{filtros lineales}. A manera de ejemplo tomemos el siguiente caso de diferencias para una serie de $Z_t$:
\begin{equation}
